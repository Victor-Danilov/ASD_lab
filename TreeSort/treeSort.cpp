...
void treeSort(int arr[], int B[], int n){
    struct Nodo *root = NULL;
    
    ct_read++;
    root = addNodeValue(root, arr[0]);
    for (int i = 1; i < n; i++)
    {
        ct_read++;
...
int main(int argc, char **argv) {
    ...
    //if (ndiv > 1)
        cout << "Dim_array,N_test,min_cmp,avg_cmp,max_cmp,min_read,avg_read,max_read,n*n,n*log(n)\n";
    ...
    for (n = max_dim / ndiv; n <= max_dim; n += max_dim / ndiv) {
        ...
        //if (ndiv > 1)
            cout << n << "," << ntests << "," << cmp_min << "," << (0.0 + cmp_avg) / ntests << "," << cmp_max << "," << read_min << "," << (0.0 + read_avg) / ntests << "," << read_max << "," << 0.0 + n * n << "," << 0.0 + n * log(n) / log(2) << "\n";
    }
